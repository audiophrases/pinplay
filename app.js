      ...
      specific += `
        ...
        <div class="wd-hint-rows">
          ${hints.slice(0, 3).map((h, i) => `
            <input data-q="${idx}" data-wd-hint="${i}" maxlength="120" value="${escapeHtml(h)}" placeholder="Hint ${i + 1} (${i === 0 ? 'vaguest, free' : i === 2 ? 'most obvious, −15%' : 'clearer, −15%'})" />
          `).join('')}
        </div>
        <p class="small">Wordle-style: the student guesses the word on an on-screen keyboard with green/yellow/grey letter feedback. Keep the question prompt generic (e.g. "Guess the word.") — put clues in the hints below instead, ordered least → most obvious. The 1st hint is free; the 2nd and 3rd each cost 15% of the points. "Guess word list" rejects made-up guesses (the answer is always accepted); use None for made-up-friendly / other-language rounds. Auto-graded, case- and accent-insensitive.</p>
      `;
    }
...
async function copyLevelTaggingPrompt() {
  ...
  const prompt = [
    ...
    '- Tag each question by the CEFR level of the language the student must understand and produce: vocabulary, grammar, sentence length, and the kind of task (recognising an answer is easier than writing one).',
    '- Only if a question tests knowledge of another subject instead of the language (general knowledge, history, science…), use the levels as difficulty steps: A1 = almost every student knows it; A2 = most students know it; B1 = typical school knowledge; B2 = needs solid study or some reasoning; C1 = only the best-read students know it; C2 = specialist or obscure.',
