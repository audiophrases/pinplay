...
function buildVideoSettingsMarkup(idx, q) {
  ...
  return `
    <div class="top-space" style="padding:.55rem; border:1px dashed var(--line); border-radius:.55rem;">
      ...
      <div class="top-space" data-video-preview-wrap="${idx}">
        ${config.src ? (config.provider === 'direct'
      ? `<video data-video-preview-el="${idx}" controls preload="metadata" src="${escapeHtml(config.src)}" style="width:100%; max-height:260px;"></video>`
      : `<iframe data-video-preview-el="${idx}" src="${escapeHtml(config.src)}" allowfullscreen referrerpolicy="strict-origin-when-cross-origin" style="width:100%; min-height:240px; border:0; border-radius:.4rem;"></iframe>`) : '<div class="small">No valid video URL yet.</div>'}
      </div>
    </div>
  `;
}
