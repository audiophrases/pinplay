...
- **Turning it on:** the Assignments row's **🎯 Adaptive** checkbox on the create page also applies to PinPlay Cup: a Cup game started while it is ticked is adaptive (needs at least 2 tagged levels). **Ticked by default since 2026-09-26** (owner's request): every newly loaded quiz with 2+ tagged levels starts ticked, and an untick sticks while the teacher keeps editing that quiz. The projected board has no toggle and never shows "Adaptive" or the level range (owner's choice, 2026-09-25; the first version had a lobby toggle on the board).
...
- **Answering:** the answer route accepts only the served question (virtual index = answers so far), records the result (right / neutral / wrong, see section 5), and serves the next one; after the Nth the attempt is `done`. Submitting needs all N answered unless the student confirmed stopping early (`force`, same as other assignments).
...
- **Teacher results:** each attempt shows a 🎯 level badge and a line with usual/final/highest level and right/answered per level; the detail view lists the served questions in order. The path marks each answer ✓ right, ✗ wrong, ~ neutral or … waiting for a grade.
...
- Polls are never served.
- **Teacher-graded questions (since 2026-09-26, owner's request):** open, image open, speaking, voice and teacher-graded text questions are tagged and served like any other (no limit per student), but each is served **once at most** and doesn't move the level when answered. When the teacher grades it, the share of the points decides: **70%+ right, 40–70% neutral, below 40% wrong** (a 1000-point essay: 750 right, 500 neutral, 250 wrong). The step is the normal one, taken from the student's and the question's levels when it was served, with the usual settling. A grade during the attempt moves the live level (and so the next questions); a grade after the attempt ended or was submitted nudges the saved level on the roster directly and counts the answer there. A regrade takes back the previous grade's effect first and is not a new answer. All grading views address the question by its quiz position (the per-student list keeps serve order and adds `servedIndex`), so a grade from the by-question view and from the per-student view land on the same answer.
