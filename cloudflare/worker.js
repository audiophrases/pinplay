  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/player/voice-upload' && request.method === 'POST') {
      ...
      if (pin && playerId) {
        ...
        // Assignment mode: verify that this attempt exists
        const assignStub = env.ROOMS.get(env.ROOMS.idFromName(ASSIGNMENTS_DO_NAME));
        const verifyResp = await assignStub.fetch(
          `https://room/assignments/state?code=${encodeURIComponent(code)}&attemptId=${encodeURIComponent(attemptId)}`,
          { method: 'GET' },
        );
        if (!verifyResp.ok) return withCors(verifyResp);
      } else {
        return json({ error: 'Provide pin+playerId (live) or code+attemptId (assignment).' }, 401);
      }
    ...
    if (url.pathname === '/api/assignments/delete-attempt' && request.method === 'POST') {
      ...
      const stub = env.ROOMS.get(env.ROOMS.idFromName(ASSIGNMENTS_DO_NAME));
      return withCors(await stub.fetch('https://room/assignments/delete-attempt', {
        method: 'POST',
        body: JSON.stringify({ code, attemptId }),
      }));
    }
  ...
  async fetch(request) {
    try {
      ...
      if (url.pathname === '/assignments/check-status' && request.method === 'POST') {
        ...
        // Need the assignment base + ALL attempts for this code (to find the
      ...
      if (url.pathname === '/assignments/state' && request.method === 'GET') {
        ...
        // Two single-row reads instead of loadAssignmentsMap (which scanned
      ...
      if (url.pathname === '/assignments/focus-event' && request.method === 'POST') {
        ...
        // Hot path during exam mode: fires on every tab focus/blur. Has to
        // be cheap. Single-row attempt read; no need to touch the assignment
        // base at all here (we don't validate any base field).
        const attempt = await this.state.storage.get(`a:${code}:t:${attemptId}`);
        if (!attempt) return json({ error: 'Attempt not found.' }, 404);
        ...
        attempt.updatedAt = Date.now();
        // Intentionally NOT updating assignment.updatedAt here — focus events
        // are high-frequency and don't warrant a full base-row write each time.
        await saveAttempt(this.state.storage, code, attemptId, attempt);
...
// Scoped list — reads only `a:<code>:t:<*>` rows, NOT the whole table.
// Used in place of loadAssignmentsMap when a route needs all attempts for
// one specific assignment (was the second-biggest DO row-read offender after
// loadAssignmentsMap itself).
async function loadAttemptsForCode(storage, code) {
  const prefix = `a:${code}:t:`;
...
const ARENA_AVATAR_PARTS = { skin: 20, hair: 29, hairColor: 25, eyes: 37, mouth: 41, glasses: 17, hat: 29, shirt: 28, head: 17 };
...
const ARENA_POWER_MS = 60 * 60 * 1000; // no power timer either   // choose a power (incl. ~1s reveal beat), else auto-pick
const ARENA_TARGET_MS = 60 * 60 * 1000; // untimed steal target pick
